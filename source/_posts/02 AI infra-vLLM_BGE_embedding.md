...
这套 embedding 服务用于企业知识库（LightRAG）的文档入库和检索。最初我用 Ollama 单机运行 BGE-M3：部署很快，但持续入库时并发稍高就会出现请求失败或服务无响应。为了保证任务跑完，客户端最终只能使用 `batch_size=2`、单并发，并在每个 batch 后设置 `sleep_interval=2.0`。

这套配置能换来稳定，却把 embedding 阶段变成了由休眠时间主导的串行任务：入库耗时难以反映模型和 GPU 的真实能力，重试机制也没有在实际压力下得到验证。于是我将服务迁移到 vLLM，希望解决三个问题：

1. 长时间运行稳定，不再依赖人为休眠；
2. 提供 OpenAI 兼容的 `/v1/embeddings` 接口，尽量减少客户端改动；
3. 能用统一口径评估 QPS、吞吐和延迟分位数。
...
2. **验证向量兼容性。** 如果 Ollama 与 vLLM 使用同一模型权重，先抽样比较两端输出的维度、归一化方式和数值一致性；存在差异时再整体重建向量索引，避免在同一索引中混用不同分布的向量。
...
5. **补做端到端压测。** 用一批真实文档走完整 LightRAG 流程，分别记录 embedding、LLM、数据库与总耗时，再评估迁移对业务的最终收益。

## 总结

这次迁移的价值，不只是把 Ollama 换成 vLLM，而是让 embedding 服务从“依赖 sleep 保持稳定”变成“可以量化容量并据此调参”。在 RTX 4090、512 token 输入下，BGE-M3 的服务吞吐上限约为 350～365 texts/s；当前推荐从 `batch_size=16、concurrency=4` 起步，再根据生产环境的延迟目标和 GPU 共存情况复测调整。
